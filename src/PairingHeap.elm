...
module PairingHeap exposing
  (PairingHeap
  , empty, findMin, merge, insert, deleteMin, fromList, toSortedList
  , singleton, isEmpty
  )

-- Taken from https://github.com/rhofour/elm-pairing-heap
-- since the packaged version doesn't support elm 0.16.
...
isEmpty : PairingHeap comparable a -> Bool
isEmpty heap =
  case heap of
    Empty ->
      True
...
singleton : comparable -> a -> PairingHeap comparable a
singleton k v =
  empty |> insert (k, v)
